  ...
  std::vector<int> & occlusion_ratios)
{
  ...
  auto camera2cloud_opt = managed_tf_buffer.getTransform<Eigen::Matrix4f>(
    camera_info_msg->header.frame_id, cloud_msg->header.frame_id, camera_info_msg->header.stamp,
  ...
  autoware::universe_utils::transformPointCloudFromROSMsg(
    *cloud_msg, cloud_camera, *camera2cloud_opt);
